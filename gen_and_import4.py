...
print(f"生成 {days} 天数据，{len(equips)} 台设备，共 {total_points} 条记录，batch={BATCH}", flush=True)
...
for d in range(days):
    ...
    for p in range(points_per_day):
        ...
        for equ in equips:
            ...
            vals = f"'{ts_str}', {a}, {b}, {e}, {h}, {j}"
            batch.append(f"INSERT INTO `{equ}` (ts, a, b, e, h, j) VALUES ({vals})")
            
            if len(batch) >= BATCH:
                multi_sql = "; ".join(batch)
                r = exec_sql(multi_sql)
                if r.get("code", 0) != 0:
                    print(f"\n❌ 失败 ({written}): {r.get('desc', r)[:200]}", flush=True)
                    break
                written += len(batch)
...
if batch:
    ...
    if r.get("code", 0) == 0:
        ...
        print(f"\n❌ 最后一批失败: {r.get('desc', r)[:200]}", flush=True)
